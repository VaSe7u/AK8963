 ...
 *
 * This example uses AK8963 in SPI mode with the Arduino's SPI
 * library.
 *
 * Created May 4, 2018
 * by Vasil Kalchev
...
// Random device ID, so we can recognize it later.
const byte magId = 0x0C;
...
bool readSPI(uint8_t device, uint8_t registerAddress,
             uint8_t *data, uint8_t size);
bool writeSPI(uint8_t device, uint8_t registerAddress,
              uint8_t *data, uint8_t size);
...
void setup() {
  ...
  mag.attachInterface(readSPI, writeSPI);
